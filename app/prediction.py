...
# raw_data_processing()
from app.yolo_pipeline import marked_mri_processing, yolo_train
import os
import pydicom
import numpy as np
import dicom2nifti
...
def example_case(path_to_image, path_to_model,to_csv = False,save_image = False):
    ...
    :param to_csv:
    :param save_image:
    :return: Предугадывает одну картику и визуализирует.
    """
    if not to_csv:
...
def folder_creator():
    name = 'survey'
    path_to_folder_research = os.path.join(path_routing.database_path,'for_test')
...
def get_volume(path_to_dicom,path_to_model,path_to_final_folder):
    ...
    to_dicom_T1 = os.path.join(path_to_final_folder, 'DICOM_T1')
    to_nifti = os.path.join(path_to_final_folder,'NIFTI')
    to_nifti_T1 = os.path.join(path_to_final_folder, 'NIFTI_T1')
    ...
    for root, dirs, files in os.walk(path_to_dicom):
        ...
        for fname in files:
            ...
            if 'T1'.lower() in dicom_content.SeriesDescription.lower() and (
                    all(orientation[:3] == [1, 0, 0]) and all(orientation[3:] == [0, 1, 0])):
                file_path_for_nifti = deepcopy(file_path)
                shutil.copy(file_path_for_nifti, to_dicom_T1)

    #перевод DICOM в NIFTI
    dicom2nifti.convert_directory(to_dicom, to_nifti, compression=True, reorient=True)
    dicom2nifti.convert_directory(to_dicom_T1, to_nifti_T1, compression=True, reorient=True)
...
def all_surveys(path_to_all_research):
    """
    :param path_to_all_research: Path for model to folder with surveys
    :return:
    """
    for folder_ in os.listdir(path_to_all_research):
        path_to_one_research = os.path.join(path_to_all_research,folder_)
        get_volume(path_to_one_research,os.path.join(path_routing.project_dir,'metrics_26.03.24/weights/best.pt'),folder_creator())


all_surveys(path_routing.prediction_path) #сюда  путь до исследований